...
public class TCHS032_1 {
    public int winner(String[] boulders, int distance) {
        int bestTime = Integer.MAX_VALUE;
        int bestIdx = 0;
        for (int i = 0; i < boulders.length; i++) {
            int t = time(convert(boulders[i]), distance);
            if (t < bestTime) {
                bestTime = t;
                bestIdx = i;
            }
    ...
    int time(int[] sides, int distance) {
        int d = 0;
        int i = 0;
        int secs = 0;
        while(d < distance) {
...
//        TCHS032_1 t = new TCHS032_1();
//        int res = t.winner(new String[]{"9191", "9911"}, 38);
