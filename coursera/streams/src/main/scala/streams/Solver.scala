...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    val newNeighbors = for {
      (b, moves) <- initial 
      nwh = neighborsWithHistory(b, moves)
      n  <- newNeighborsOnly(nwh, explored)
      } yield n
    if (newNeighbors.isEmpty) Stream() 
    else initial #::: from(newNeighbors, explored ++ (newNeighbors map (_._1)))
  } 
