...
abstract class TweetSet {
  ...
  //  else if (this contains that.head) this union that.tail
  ...
  def ascendingByRetweet0(acc: Trending): Trending = 
    if (this.isEmpty) acc
      ...
      val min = findMin
      (this remove min).ascendingByRetweet0(acc + min)
    }
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filter0(p: Tweet => Boolean, accu: TweetSet): TweetSet = {
    val accu1 = if (p(elem)) accu incl elem else accu
    left.filter0(p, accu1) union right.filter0(p, accu1)
  } 
...
object GoogleVsApple {
  ...
  def filterTweets (ss: Seq[String]) = TweetReader.allTweets filter { t => 
    ss exists {t.text.contains}
  } 
